...
class ImageUtils {
  static Future<File?> compressImage(File file) async {
    try {
      ...
      int originalSizeKB = await file.length() ~/ 1024;
      ...
      int quality = 85;
      const int maxSizeKB = 102400; // 100 MB = 102400 KB
      ...
      // Reduce quality until image becomes <= 100MB
      while (fileSizeKB > maxSizeKB && quality > 10) {
        quality -= 10;

        final XFile? reCompressed =
        ...
        compressedFile = File(reCompressed.path);
        fileSizeKB = await compressedFile.length() ~/ 1024;
      }

      print("Final compressed image size: $fileSizeKB KB");
