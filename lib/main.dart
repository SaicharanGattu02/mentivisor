...
import 'dart:async';
import 'dart:convert';
// Removed: dart:io, dart:isolate, dart:ui
...
import 'package:mentivisor/services/SecureStorageService.dart';
import 'package:mentivisor/utils/AppLogger.dart';
// Removed: CrashlyticsBlocObserver
import 'package:mentivisor/utils/media_query_helper.dart';
import 'StateInjector.dart';
import 'app_routes/router.dart';
import 'package:provider/provider.dart';
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  await Firebase.initializeApp(options: DefaultFirebaseOptions.currentPlatform);
  // Web doesn't support background FCM, so this only runs on mobile
}
...
void main() {
  runZonedGuarded(
      ...
      // Only enable these features on non-web platforms
      if (!kIsWeb) {
        // API interceptors
        ApiClient.setupInterceptors();
        ...
        FirebaseMessaging.onMessage.listen((RemoteMessage message) {
          ...
          if (notification != null) {
            AppLogger.log("Foreground notification: ${notification.title}");
          }
        });
      }
