...
import '../data/Cubits/UpdateMentorProfile/update_mentor_profile_cubit.dart';
...
class UpdateProfileScreenState extends State<UpdateProfileScreen> {
  ...
  void initState() {
    super.initState();
    // context.read<MentorProfileCubit1>().getMentorProfile().then((mentorData) {
    //   if (mentorData != null) {
    //     final data = mentorData.data;
    //     setState(() {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: BlocConsumer<UpdateMentorProfileCubit, UpdateMentorProfileStates>(
            listener: (context, state) {
              if (state is UpdateMentorProfileLoaded) {
                context.read<MentorProfileCubit1>().getMentorProfile();
                CustomSnackBar1.show(context, state.successModel.message ?? "");
                context.pop();
              } else if (state is UpdateMentorProfileFailure) {
                CustomSnackBar1.show(context, state.error ?? "");
              }
            },
            builder: (context, state) {
              final isLoading = state is UpdateMentorProfileLoading;
              return SizedBox(
                  ...
                  onPlusTap: () {
                    if (_formKey.currentState!.validate() && !isLoading) {
                      final data = {
                        ...
                        "college_id": _nameController.text.trim(),
                      ...
                      context.read<UpdateMentorProfileCubit>().updateMentorProfileDetails(data);
                    }
                  },
                ),
              );
