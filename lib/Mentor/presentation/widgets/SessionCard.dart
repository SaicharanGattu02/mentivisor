...
class SessionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime sessionStartDateTime = DateFormat(
      "dd MMM yyyy h:mm a",
    ).parse("$sessionDate $sessionStartTime");

    DateTime sessionEndDateTime = DateFormat(
      "dd MMM yyyy h:mm a",
    ).parse("$sessionDate $sessionEndTime");

    bool isSessionOver = DateTime.now().isAfter(sessionEndDateTime);
    bool isSessionLive =
        DateTime.now().isAfter(sessionStartDateTime) &&
        DateTime.now().isBefore(sessionEndDateTime);

    // Responsive image size based on screen width
    double imageSize = SizeConfig.screenWidth < 600 ? 60 : 70;
    ...
    return InkWell(
      ...
      child: Card(
        ...
        child: Container(
          padding: EdgeInsets.all(SizeConfig.screenWidth < 600 ? 10 : 12),
          child: Column(
            ...
            children: [
              Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Expanded(
                        // Use Expanded for responsive text area
                        child: Padding(
                  ...
                  if (attachment.isNotEmpty && status == 'upcoming') ...[
                    const SizedBox(height: 8),
                    Row(
                      mainAxisSize:
                          MainAxisSize.min, // Keep min to avoid expansion
                      children: [
                        Image.asset(
              ...
              if (status == 'upcoming' && !isSessionOver) ...[
                Row(
                  children: [
                    /// CHAT BUTTON (only when live)
                    if (isSessionLive) ...[
                      Expanded(
                        child: Container(
                          height: 48,
                ...
                BlocConsumer<SessionCompleteCubit, SessionCompleteStates>(
                  ...
                  builder: (context, state) {
                    final bool isThisCardLoading =
                        state is SessionCompletdLoading &&
              ...
              //     // if (status == 'upcoming') ...[
              //     if (status == 'upcoming' && isSessionLive) ...[
              //       Builder(
              //         builder: (context) {
              ...
              //     // if (status == 'upcoming') ...[
              //     if (status == 'upcoming' && !isSessionLive && !isSessionOver) ...[
              //
              //       Expanded(
              //         child: SizedBox(
              //           height: 48,
            ...
            builder: (BuildContext context, StateSetter setState) {
              return Padding(
                padding: EdgeInsets.only(
                  ...
                  left: SizeConfig.screenWidth < 600 ? 12 : 16,
                  right: SizeConfig.screenWidth < 600 ? 12 : 16,
                  top: 16,
                ),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          'Report Mentee',
                          style: TextStyle(
                            fontSize: SizeConfig.screenWidth < 600 ? 18 : 20,
                            fontFamily: 'segeo',
                    ...
                    const SizedBox(height: 8),
                    Flexible(
                      // Add Flexible to handle long lists on small screens
                      child: Column(
                        children: _reportReasons.map((String reason) {
                          return RadioListTile<String>(
                            ...
                            contentPadding: EdgeInsets.symmetric(
                              horizontal: SizeConfig.screenWidth < 600 ? 4 : 8,
                              vertical: 0,
                            ),
                          );
                        }).toList(),
                    ...
                    if (_selected == 'Other') ...[
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintStyle: TextStyle(fontFamily: 'segeo'),
